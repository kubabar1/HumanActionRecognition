...
class TestLSTMSimple(unittest.TestCase):
    ...
    def test_run_training_evaluate_fit(self):
        ...
        test_train(training_data, training_labels, validation_data, validation_labels, self.test_results_path)

        lstm_simple_model = test_load_model(os.path.join(self.test_results_path, generated_model_name))

        test_evaluation(lstm_simple_model, test_data, test_labels, self.test_results_path)

        test_fit(lstm_simple_model, test_data, test_labels)
...
def test_train(training_data, training_labels, validation_data, validation_labels, test_results_path):
...
def test_load_model(generated_model_path):
    return load_model(generated_model_path, berkeley_mhad_classes, video_pose_3d_kpts)


def test_evaluation(lstm_simple_model, test_data, test_labels, results_path, print_results=False):
...
def test_fit(lstm_simple_model, test_data, test_labels, print_results=False):
